...
class VarietySpecialMapper(_PluginBase):
                                        ...
                                        "content": [
                                            {
                                                "component": "VExpansionPanels",
                                                "props": {"multiple": True, "popout": True},
                                                "content": [self._build_common_type_panel(type_key) for type_key in common_type_keys]
    ...
    def _parse_interactive_form_config(self, config: Dict[str, Any]) -> Tuple[Dict[str, Dict[str, Any]], List[Dict[str, Any]]]:
        current_rules = self._normalize_rules(self._rules or self._default_rules_data())
        common_types: Dict[str, Dict[str, Any]] = {}
        current_common_types = self._normalize_common_types(self._common_types or self._default_common_types())
        common_type_keys = self._get_common_type_keys(current_common_types)

        for type_key in common_type_keys:
            if bool(config.get(f"common_type_{type_key}_delete")):
                continue
            common_types[type_key] = {
                "label": str(config.get(f"common_type_{type_key}_label") or self._type_label(type_key)).strip() or self._type_label(type_key),
        ...
        if new_common_type_label:
            new_type_key = self._make_common_type_key(new_common_type_label, common_types)
        ...
        deleted_type_keys = set(common_type_keys) - set(common_types.keys())
        current_rules = self._prune_deleted_type_keys(current_rules, deleted_type_keys)
    ...
    def _normalize_common_types(self, common_types: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        for type_key, type_conf in (common_types or {}).items():
            if not type_key:
                continue
            normalized[type_key] = {
                "label": str((type_conf or {}).get("label") or self._builtin_type_labels().get(type_key) or type_key).strip()
                or self._builtin_type_labels().get(type_key)
                or type_key,
    ...
    def _make_common_type_key(self, label: str, common_types: Dict[str, Dict[str, Any]]) -> str:
        target = str(label or "").strip()
        if not target:
            return "custom_1"

        for type_key, builtin_label in self._builtin_type_labels().items():
            if builtin_label == target and type_key not in common_types:
                return type_key

        slug = re.sub(r"[^0-9A-Za-z_\-]+", "_", target).strip("_").lower()
        if not slug:
            slug = "custom"
        if slug[0].isdigit():
            slug = f"type_{slug}"
        ...
        candidate = slug
        index = 2
        while candidate in common_types:
            candidate = f"{slug}_{index}"
            index += 1
        return candidate

    def _prune_deleted_type_keys(self, rules: List[Dict[str, Any]], deleted_type_keys: set) -> List[Dict[str, Any]]:
    ...
    def _build_form_model(self) -> Dict[str, Any]:
        ...
        for type_key in common_type_keys:
            ...
            model[f"common_type_{type_key}_label"] = str(common_conf.get("label") or self._type_label(type_key)).strip() or self._type_label(type_key)
    ...
    def _build_type_panel(self, title: str, source_model: str, tmdb_model: str) -> Dict[str, Any]:
        return {
            ...
            "content": [
                ...
                {
                    "component": "VExpansionPanelText",
                    "content": [self._build_type_panel_content(source_model, tmdb_model)],
                },
            ],
        }
                    ...
                    "content": [
                        {
                            ...
                            "props": {
                                "type": "info",
                                "variant": "tonal",
                                "text": "这里新增的是分类名本身，比如新增“纯享”“夜聊”这一层；下面的标签词可以一起填好。",
                            },
                        },
                        {
                            "component": "VTextField",
    ...
    def _type_label(self, type_key: str) -> str:
        common_conf = self._common_types.get(type_key) or {}
        return str(common_conf.get("label") or self._builtin_type_labels().get(type_key) or type_key)

    @classmethod
    def _get_common_type_keys(cls, common_types: Optional[Dict[str, Any]] = None) -> List[str]:
        target = cls.COMMON_TYPES_TEMPLATE if common_types is None else common_types
        ordered: List[str] = []
