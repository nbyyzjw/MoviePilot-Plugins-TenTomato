...
class IdentifierHelper(_PluginBase):
    ...
    # 插件图标
    plugin_icon = "https://raw.githubusercontent.com/nbyyzjw/MoviePilot-Plugins-TenTomato/main/icons/Identifier.png"
    # 插件版本
    ...
    _entries = []
    _categories = []
    _category_tree_storage_key = "identifier_category_tree"
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        return [
            ...
            {
                "path": "/save_category_tree",
                "endpoint": self.save_category_tree,
                "methods": ["POST"],
                "auth": "bear",
                "summary": "保存分类树结构",
                "description": "保存空分类和子分类结构，支持即时保存界面"
            },
            {
    ...
    def _normalize_category_tree(self, tree_data: Any) -> List[Dict[str, Any]]:
        """规范化分类树数据，仅保留分类和子分类名称。"""
        normalized = []
        ...
        for category in tree_data:
            ...
            category_name = str(category.get('name') or category.get('id') or '').strip() or '未分类'
            ...
            raw_sub_categories = category.get('subCategories') or category.get('sub_categories') or []
            if isinstance(raw_sub_categories, list):
                for sub_category in raw_sub_categories:
                    if not isinstance(sub_category, dict):
                        continue
                    sub_category_name = str(
                        sub_category.get('name')
                        or sub_category.get('id', '').split('/', 1)[-1]
                        or ''
                    ).strip()
                    if not sub_category_name or sub_category_name in seen_sub_categories:
                        continue
                    seen_sub_categories.add(sub_category_name)
    ...
    def _serialize_category_tree(self, tree_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        normalized = self._normalize_category_tree(tree_data)
        return [
            {
                'id': category['id'],
                'name': category['name'],
    ...
    def _get_saved_category_tree(self) -> List[Dict[str, Any]]:
        try:
            return self._normalize_category_tree(self.get_data(self._category_tree_storage_key) or [])
        except Exception as e:
            logger.warning(f"读取分类树缓存失败: {e}")
            return []

    def _save_category_tree_data(self, tree_data: List[Dict[str, Any]]) -> None:
        try:
            self.save_data(self._category_tree_storage_key, self._serialize_category_tree(tree_data))
        except Exception as e:
            logger.warning(f"保存分类树缓存失败: {e}")

    def _merge_hierarchical_structure(
        self,
        identifiers: List[Dict[str, str]],
        tree_data: Optional[List[Dict[str, Any]]] = None
    ) -> List[Dict[str, Any]]:
        """合并分类树与识别词，保留空分类结构。"""
        categories = self._normalize_category_tree(tree_data or [])
        category_map = {category['name']: category for category in categories}

        for item in identifiers:
            tag = str(item.get('tag') or '未分类')
            parts = [part.strip() for part in tag.split('/') if str(part).strip()]
            category_name = parts[0] if parts else '未分类'
            sub_category_name = '/'.join(parts[1:]) if len(parts) > 1 else None
            ...
            if sub_category_name:
                sub_category = next(
                    (sub for sub in category['subCategories'] if sub['name'] == sub_category_name),
                    None
                )
                if not sub_category:
                    sub_category = {
        ...
        try:
            ...
            try:
                ...
                if isinstance(saved_data, list) and len(saved_data) == len(list_data):
                    ...
                    try:
                        parsed_identifiers = self.extract_identifier(final_data)
                        merged_tree = self._merge_hierarchical_structure(
                            parsed_identifiers,
                            self._get_saved_category_tree()
                        )
                        self._save_category_tree_data(merged_tree)
                    except Exception as category_error:
                        logger.warning(f"同步分类树缓存失败: {category_error}")
                    return {
                        "code": 0,
    ...
    def get_hierarchical_data(self) -> Dict[str, Any]:
        ...
        try:
            identifiers = self.extract_identifier()
            hierarchical_data = self._merge_hierarchical_structure(
                identifiers,
                self._get_saved_category_tree()
            )
            return {
    ...
    async def save_category_tree(self, request: Request) -> Dict[str, Any]:
        """保存分类树结构，支持空分类和空子分类。"""
        try:
            payload = []
            body = await request.body()
            if body:
                try:
                    ...
                    if isinstance(body_data, dict):
                        payload = body_data.get('categories') or body_data.get('data') or []
                    elif isinstance(body_data, list):
                        payload = body_data
                except json.JSONDecodeError:
                    logger.warning("save_category_tree 请求体不是合法 JSON，忽略")

            normalized_tree = self._serialize_category_tree(payload)
            self._save_category_tree_data(normalized_tree)
            return {
                "code": 0,
                "message": f"分类结构保存成功，共{len(normalized_tree)}个主分类",
                "data": normalized_tree
            }
        except Exception as e:
            logger.error(f"保存分类结构失败: {e}")
            return {
                "code": 1,
                "message": f"保存分类结构失败: {str(e)}",
                "data": []
            }
